 ...
 * @param {number=} offset
 */
function InvalidOperand(operator, offset) {
 ...
 * @param {*} rightValue
 */
function assignProperty(operator, object, property, rightValue, offset) {
    if(object == null) {
        throw new PropertyOfNull(property);
    }
    ...
    switch(operator) {
        ...
        case "|="  : result = bitor(leftValue, rightValue); break;
 ...
 * @param {number=} offset
 */
function bitor(left, right, offset) {
...
File.prototype.write = function(value, callback) {
    ...
    if(this.sync) {
        ...
        fs.read(this.fd, value, 0, value.length, null, callback);
    }
};
