 ...
 * @param {number=} offset
 */
function InvalidUnaryOperand(operator, value, offset) {
...
function call(receiver, func) {
    var args = Array.prototype.slice.call(arguments, 2);
    if(receiver === null) {
        var res = func.apply(null, args);
    } else {
        res = receiver[func].apply(receiver, args);
    }
...
var FLUSH = new File("", 1, true);

FLUSH.write = function(value, callback) {
    callback();
};

FLUSH.close = FLUSH.writeStart = function noop() {};
...
function print() {
    for(var i = 0; i < arguments.length; i++) {
        if(Buffer.isBuffer(arguments[i]) === false) {
...
function timeout(milliseconds, block) {
    setTimeout(function() {
        block();
    }, milliseconds);
}
