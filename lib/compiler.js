...
Compiler.prototype.Block = function(node) {
    ...
    switch(node.statements.length) {
        case 0:
            ...
            return;
        case 1:
            this.singleLineBlock(node.statements);
            return;
        default:
            this.multiLineBlock(node.statements);
    }
};

Compiler.prototype.singleLineBlock = function(nodes) {
    var varsPosition = this.write("");
    this.write("return ");
    this.pushScope();
    this.c(nodes[0]);
    ...
    if(vars.length !== 0) {
        this.out[varsPosition] = "var " + vars.join(", ") + "; ";
    }
};

Compiler.prototype.multiLineBlock = function(nodes) {
    ...
    var scope = this.popScope();
    if(nodes[nodes.length - 1].type === NodeType.If) {
    ...
    var vars = this.findDeclarations(scope);
    if(vars.length !== 0) {
        this.out[varsPosition] = this.indent + indentSize + "var " + vars.join(", ") + ";\n";
    }
};
